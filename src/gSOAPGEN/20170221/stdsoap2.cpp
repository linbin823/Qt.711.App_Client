  ...
  { for (;;)
    {
...
#endif
      {
        ...
        { SOAP_SOCKLEN_T k = (SOAP_SOCKLEN_T)sizeof(soap->peer);
          ...
          r = recvfrom(sk, s, (SOAP_WINSOCKINT)n, soap->socket_flags, (struct sockaddr*)&soap->peer, (socklen_t*)&k);	/* portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h */
  ...
  for (;;)
  {
    ...
    { err = soap_socket_errno(sk);
      ...
      else if (soap->connect_timeout && (err == SOAP_EINPROGRESS || err == SOAP_EAGAIN || err == SOAP_EWOULDBLOCK))
      {
        ...
        if (!getsockopt(sk, SOL_SOCKET, SO_ERROR, (char*)&soap->errnum, (socklen_t*)&k) && !soap->errnum)	/* portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h */
          break;
        DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Could not connect to host\n"));
        if (!soap->errnum)
          soap->errnum = soap_socket_errno(sk);
...
{ SOAP_SOCKET sk;
  (void)soap;
  sk = accept(s, a, (socklen_t*)n); /* portability note: see SOAP_SOCKLEN_T definition in stdsoap2.h */
#ifdef SOCKET_CLOSE_ON_EXEC
#ifdef WIN32
#ifndef UNDER_CE
  SetHandleInformation((HANDLE)sk, HANDLE_FLAG_INHERIT, 0);
