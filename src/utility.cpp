...
unsigned int CRC16(unsigned char *puchMsg, unsigned char usDataLen){
    ...
    //unsigned short usDataLen ; //缓冲器中的字节数。 CRC产生的功能
...
bool CRC16Verify(unsigned char * data, unsigned char length){
    unsigned int tmp=CRC16(data,length-2);
    //qDebug()<<tmp<<"tmp: "<<data[length-2]+data[length-1]*256<<" "<<data[length-2]<<" "<<data[length-1];
    return tmp==data[length-2]+data[length-1]*256?true:false;
}
